...
from roboot.Roboot_0526 import Ui_mainWindow
...
from utils.torch_utils_mask_body_track import distancing,plot_dots_on_car
...
#0501更新：加入文件夹遍历，生成字典，找出区别最小的值及其对应的字典

def read_directory(directory_name,face_cut):#遍历文件夹。并判断差别最小的生成字典
    face = Facenet()
    probability = []
    i = 0
    dic = {}
    for filename in os.listdir(directory_name):
        ...
        yz = Image.open(img1)
        probability.append(face.detect_image(face_cut, yz))
        P= face.detect_image(face_cut, yz)
        # dic ={name:P}
        dic[name] = P
        i = i + 1
    return probability,P,i,dic
    print(i)
    ...
    def run(self,
        ...
        try:
            ...
            if self.source.isnumeric() or self.source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://')):
                # view_img = check_imshow()kkkkkkkkkkkkkkkk??
                    ...
                    for i, det in enumerate(pred):  # detections per image
                        ...
                        if len(det):
                            ...
                            for *xyxy, conf, cls in reversed(det):
                                face_cut = im0[int(xyxy[1].item()):int(xyxy[3].item()),
                                ...
                                LST, P, i, dic = read_directory("/mnt/code/PyQt5-YOLOv5-3.0/Verify_Imgs", face_cut)
                                # print(LST, type(LST), len(LST), i, dic)
                                max_P = min(zip(dic.values(), dic.keys()))

                                c = int(cls)  # integer class
                                statistic_dic[names[c]] += 1
                                label = f'{names[int(cls)]} {conf:.2f} {max_P[-1]}'
